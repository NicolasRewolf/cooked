# Zone (d) — sémantique des RPC et vues : « une notion = une implémentation »

> Mission Cooked du 02/09/2026, Phase 1, **lecture seule**. Mesures prises entre
> **09:53 et 15:03 (Paris)** le 02/09/2026 via MCP `execute_sql` (projet
> `mxycmjkeotrycyneacje`) et lecture du repo à `e95f3ee`. Dates en JJ/MM/AAAA,
> heures Paris. Aucune écriture, aucune PII.
...
8 constats. Le plus grave (**d-01**) est un **facteur 100** dans une RPC publiée,
introduit le 26/07/2026 par le correctif du défaut jumeau du même lot de
remédiation d'audit, vert aux contract-tests depuis 38 jours.

Trois notions du périmètre se révèlent avoir **plusieurs implémentations qui ne
renvoient pas le même chiffre** : `bounce_rate` (d-01, d-04), « contacts macro
28 j » (d-02), « fenêtre GSC 28 j » (d-03, d-07). Une quatrième, le filtre spam,
a **deux périmètres d'application** selon qu'on passe par `events_human` ou par
`cooked_events_window` (d-05).

Deux hypothèses fortes du brief sont **écartées avec preuve** : la *définition*
du contact macro est bien unique (seules les fenêtres divergent), et le spam
Baidu ne contamine **ni** les contacts macro **ni** le CPI (§ Écarté).

| ID | Sév. | Titre |
|---|---|---|
| d-01 | P0 | `behavior_pages_for_period` : bounce_rate 100× trop faible depuis le 26/07/2026 |
| d-02 | P1 | « Contacts macro 28 j » : 3 fenêtres, 183 / 193 / 195 (écart 6,6 %) |
| d-03 | P1 | `gsc_pages_overview.gsc_clicks_28d` couvre 24 j (−15,3 %) — récidive du 24/05/2026 |
| d-04 | P1 | `cooked_bounce_rate` : même nom de colonne, deux unités (0,2342 vs 34,43) |
| d-05 | P1 | Filtre spam absent d'`events_human` : 19 % des « visiteurs », jusqu'à 98,7 % sur une page |
| d-06 | P1 | `seo_to_contact_funnel.contact_rate_pct` : numérateur et dénominateur incomparables |
| d-07 | P2 | `cooked_page_index` : GSC 24 j vs Cooked 28×24 h, `current_date` en UTC |
| d-08 | P2 | Overloads ambigus et vestiges : 4 paires de doublons sémantiques |
...
Hypothèses du brief examinées et **réfutées**, avec preuve. Elles sont listées
ici précisément pour qu'elles ne soient pas ré-instruites.

1. **« La définition du contact macro n'est pas unique. »** FAUX. Le prédicat
   `cta_phone_click` + `form_submit AND form_submit_counts_as_macro(props)` est
   utilisé tel quel par les 6 routines qui comptent des contacts
   (`assisted_contacts_by_entry_path`:551, `conversion_journeys`:766,
   `dashboard_honoraires_funnel`:1921, `form_submits_per_path`:2373,
   `macro_contacts_by_path`:2922-2936, `site_macro_counts`:5281-5286, plus
   `refresh_dashboard_*`:3905 et :4089). Aucune ne réimplémente le prédicat.
   Vérification numérique le 02/09/2026 à 09:58 Paris : sur les trois fenêtres,
   `site_macro_counts` = Σ `macro_contacts_by_path` **exactement** (193=193,
   195=195, 183=183), et l'écart entre fenêtres se referme au contact près par
   décomposition journalière. → La divergence de d-02 est **100 % de fenêtre,
   0 % de définition**.

2. **« Le spam Baidu contamine les contacts macro. »** FAUX. Sur les 1 899
   sessions spam de la fenêtre 05/08→01/09, le nombre d'events
   `cta_phone_click` / `form_submit` / `cta_booking_click` est **0** (mesuré
   02/09 09:53 Paris). Les contacts sont donc propres, quelle que soit la RPC.

3. **« Le spam Baidu contamine le CPI et les entrées organiques. »** FAUX, par
   le canal. `classify_channel` (rpcs.sql) ne liste `baidu` ni dans
   `organic_other` ni ailleurs : un referrer `m.baidu.com` tombe dans la branche
   finale `else 'referral'`. Le prédicat `chan LIKE 'organic%'` employé par
   `cooked_page_index` (`orge`, :1171), `seo_to_contact_funnel` (`organic`),
   `content_performance` et `refresh_dashboard_expertises_snapshots` l'exclut
   donc mécaniquement. `n_org` est propre.

4. **« Les 3 copies littérales du filtre Baidu créent un écart de chiffre. »**
   FAUX — double filtre inoffensif. Les 3 sites (`dashboard_article_detail`:1765,
   `refresh_dashboard_expertises_snapshots`:3779, `refresh_dashboard_snapshots`:3985)
   testent `referrer_hostname IS DISTINCT FROM 'm.baidu.com' AND … 'baidu.com'`,
   soit exactement les deux hostnames de `cooked_is_spam_referrer` (rpcs.sql:1082).
   Les deux refresh lisent `_cooked_ev` produit par `cooked_events_window(w,'clean')`,
   déjà filtré ; `dashboard_article_detail` lit `events_human` et le filtre y est
   donc **nécessaire**, pas redondant. Nuance à conserver : les copies filtrent
   TOUS les events, le filtre central seulement `name = 'pageview'` — les copies
   filtrent donc légèrement PLUS. Dette de duplication (P3), pas un écart de chiffre.

5. **« Il reste des casts bruts `occurred_at::date` dans les corps prod. »** FAUX.
   Balayage des 122 routines par `pg_get_functiondef` avec le motif
   `occurred_at\s*::\s*date` (02/09 ~09:57 Paris) : **0 occurrence**. La règle
   dure de CLAUDE.md tient sur ce point précis. En revanche `current_date` (2
   fonctions) et `now() AT TIME ZONE` brut (8 fonctions) subsistent — c'est le
   fond de d-06 et d-07.

6. **« `page_reads` est sémantiquement redondante avec `dashboard_article_detail`
   ou `content_performance`. »** FAUX. `page_reads` renvoie un grain
   **session×path** (`session_id, path, dwell_s, scroll_pct, session_pageviews,
   retained, source`) que ni l'une ni l'autre n'expose :
   `dashboard_article_detail` renvoie un objet agrégé par page,
   `content_performance` des agrégats par `page_type × theme`. Le défaut de
   `page_reads` est ailleurs (deux overloads, aucun consommateur, exposition
   `anon` — d-08c et zone h), pas dans une redondance sémantique.

7. **« `contacts assistés` a plusieurs implémentations. »** FAUX, déjà établi en
   Phase 0 et confirmé par `grep` : `dashboard_assisted_quarter`:1804 et
   `refresh_dashboard_resources_assisted`:3944,3948 appellent tous deux
   `assisted_contacts_by_entry_path`. Une seule implémentation. (Le problème de
   cette famille est un timeout, pas une divergence — hors zone d.)

---

## Non vérifiable, et pourquoi

1. **Effet chiffré du désalignement de fenêtres sur le CPI page par page (d-07).**
   Le brief interdit d'appeler `cooked_page_index` (timeout MCP constaté). J'ai
   donc établi le mismatch par lecture du corps et mesuré le volume GSC manquant
   au niveau site (−822 clics, −15,3 %), sans pouvoir dire combien de pages
   changent de grade.

2. **Effet chiffré sur `seo_to_contact_funnel.contact_rate_pct` (d-06).** La RPC
   appelle `conversion_journeys(28)`, coûteuse ; le brief proscrit les appels
   longs et le connecteur coupe à ~60 s. Le constat repose sur le corps prod et
   sur la mesure du `TimeZone` = UTC, pas sur une exécution.

3. **Recoupement avec `conversion_weekly` / `cooked_weekly_conversions_snapshot`.**
   Ces routines écrivent (`cooked_weekly_conversions_snapshot` est explicitement
   dans la liste d'interdits du brief). Je n'ai donc pas comparé la somme des
   semaines de `conversion_weekly` aux 183/193/195 de d-02. C'est le seul
   consommateur de contacts macro que je n'ai pas placé sur la carte des fenêtres
   — d'autant plus opaque que la migration qui les crée
   (`20260807224552_weekly_conversion_pages_routine`) est la seule migration prod
   sans aucun miroir local (baseline §1).

4. **Ancienneté exacte de d-04 et d-05 en prod.** `supabase/migrations/` n'est
   pas un miroir fiable (212 versions prod / 162 fichiers locaux, baseline §2.5),
   et `supabase/rpcs.sql` diverge de la prod sur 2 fonctions + 6 manquantes. J'ai
   daté d-01 et d-04 par le contenu des deux migrations du 25 et 26/07/2026 qui
   sont, elles, présentes localement et cohérentes avec le corps prod lu — mais je
   n'ai pas pu confirmer par `schema_migrations` que ce sont bien les dernières à
   avoir touché ces corps.

5. **Réalité « bot » des 1 899 sessions Baidu (d-05).** Je mesure leur volume et
   leur signature (1 pv / 1 anon / 1 session, 0 page_exit, 0 scroll, ~10 ticks) ;
   la qualification « spam » est héritée de la décision projet, non re-instruite.

6. **Conséquence sur le classement de `gsc_pages_overview` (d-03).** Le −15,3 %
   est un total site ; établir qu'une paire de pages bascule effectivement dans
   le tri aurait demandé une comparaison page à page des deux fenêtres, non faite
   faute de budget.
