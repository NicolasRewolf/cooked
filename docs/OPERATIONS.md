...
| `cooked-alerts-hourly` | `15 * * * *` | Table `alerts` — **driver à découverte dynamique depuis le 23/08/2026** (ADR-0002) : toute fonction `alert_rule_*()` du catalogue est exécutée, une règle qui plante devient une alerte `<règle>_crashed`. La fraîcheur de TOUTES les sources vit dans le **registre `SELECT * FROM freshness_contract`** (kinds `<source>_stale` / `<source>_gap`), plus les règles métier (pipeline_dead, double-embed, RPCs, attribution, cpi_drop, tracker_drift, gsc_ingest_missed) et l'**escalade** `alert_rule_warn_escalation` (warn ininterrompu ≥ 5 j → critical ; acker suspend). Les `critical` **poussent sur ntfy** (topic dans `cooked_config`), sauf si la dernière alerte du kind est ackée · `SET statement_timeout='300s'` |
| `gsc-daily-ingest` / `dfs-weekly-sync` | GitHub Actions | GSC quotidien 06:00 UTC (`--months 2` depuis T-02 — la fenêtre mois-calendaire perdait les fins de mois) ; DFS hebdo lundi 07:00 UTC (échec = run rouge). Les 2 notifient ntfy en échec — **réellement actif depuis le 22/08/2026 seulement** : le secret GitHub `NTFY_TOPIC` n'avait jamais été posé, les steps `notify-failure` étaient inertes |
| `gbp-daily-ingest` | GitHub Actions | Google Business Profile quotidien 05:30 UTC, fenêtre 30 j (lag ~J-4, la queue rembourrée à zéro est coupée par le script) → `gbp_daily`. Notifie ntfy en échec (secret `NTFY_TOPIC` posé le 22/08/2026). ⚠️ **Le credential est un ADC utilisateur : Google exige une reauth périodique** — pannes du 30/07-04/08 puis du 08/08-22/08/2026 (`gcloud auth application-default login --scopes=…business.manage,…cloud-platform` puis secret `GBP_CREDENTIALS_B64` re-poussé). La fraîcheur est surveillée par le registre (`gbp_daily_stale`, warn > 7 j / critical > 14 j). Parade durable : client OAuth dédié (voie 2 de `scripts/gbp_ingest.py`) |
