-- Perf : contacts_daily scannait events_human (vue à anti-joins) sur 90j → timeout.
-- Remplacé par events + filtre name sélectif + anti-joins inline (même sémantique
-- que events_human, vérifié : 14 = référence macro_contacts_by_path sur 90j).
-- Seule la sous-requête contacts_daily change vs dashboard_trend_visitors_align_kpi.
-- (État final de refresh_dashboard_snapshots pour le lot séries journalières.)
  ...
  FOREACH w IN ARRAY windows LOOP
    ...
    SELECT w,
      (SELECT array_agg(COALESCE(v.uv,0)::numeric ORDER BY ds.d)
         ...
         LEFT JOIN (SELECT d, COUNT(DISTINCT anonymous_id) uv FROM _evc WHERE d BETWEEN lns AND lne GROUP BY d) v ON v.d = ds.d),
