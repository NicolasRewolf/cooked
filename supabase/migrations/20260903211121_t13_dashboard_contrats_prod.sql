-- T-13 (mission 02/09/2026, #114) — dashboard : contrats depuis la prod, RPC sous contract-test
-- avec budget, colonnes snapshot NOT NULL, OTP sans création de compte. Constats g-01 (P1),
-- g-04, g-06, g-08. Invariant I8.
--
-- Mesure avant (03/09/2026 23:05 Paris) :
--   · contrat RPC ↔ Zod : 2 RPC sur 16 (check_dashboard_contracts.py compare deux fichiers du
--     repo, jamais la prod — l'incident /seo du 25/07 (16 colonnes prod / 20 Zod) restait possible).
--   · latest_rpc_health() : 23 RPC ; les 16 dashboard_* y sont depuis T-08 mais aucune n'avait
--     tourné en nocturne (première nuit : 04/09 03:30 UTC) ; aucun budget de durée.
--   · nullabilité : dashboard_resources_snapshot 3/34 colonnes NOT NULL, dashboard_kpis_snapshot
--     2/22 ; 31 champs Zod non nullables sur des colonnes nullables ; 0 NULL observé
--     (126 + 28 + 2 + 2 lignes) — le contrat tenait par la discipline du refresher.
--   · durées mesurées à 23:00 : dashboard_article_detail 1 184 ms (rolling_28), 694 ms et 9 ms
--     (rolling_90, deux articles) — 34 s max le 25/07 ; honoraires_funnel 1 595 ms ; seo_by_query 2 823 ms.
--   · cooked_ci_ro : 0 EXECUTE sur dashboard_* (le contrat depuis la prod ne pouvait pas s'échantillonner).
--
-- Changement :
--   1. NOT NULL (+ DEFAULT 0 / false / 'C') sur les colonnes que les schémas Zod exigent non nulles :
--      le schéma garantit ce que le refresher promettait. Un refresher qui produirait un NULL
--      échouera (refresh_step_failed_* + refresh_after_gsc_stale, T-11) au lieu de casser la page.
--   2. GRANT EXECUTE sur les 16 dashboard_* à cooked_ci_ro (lecture seule, agrégats sans PII) :
--      scripts/generate_dashboard_contracts.py écrit contracts/dashboard_rpc_columns.json depuis
--      la prod et échantillonne un appel réel par RPC, parsé par les schémas Zod en CI (prod-drift.yml).
--   3. contract-test dashboard_rpc_budget (durées du run courant : 20 s article_detail, 15 s
--      honoraires_funnel, 8 s seo_by_query, 5 s les autres).
-- Hors SQL, même PR : dashboard/src/data/rpc-contract.test.ts (clés + nullabilité + échantillons),
-- shouldCreateUser:false (lib/otp.ts, testé), workflows prod-drift / dashboard-contract / sql-contracts.

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. Nullabilité des tables snapshot (0 NULL observé le 03/09/2026 : l'ALTER échouerait sinon)
-- ─────────────────────────────────────────────────────────────────────────────
ALTER TABLE public.dashboard_resources_snapshot
  ALTER COLUMN unique_visitors SET DEFAULT 0, ALTER COLUMN unique_visitors SET NOT NULL,
  ALTER COLUMN pageviews SET DEFAULT 0, ALTER COLUMN pageviews SET NOT NULL,
  ALTER COLUMN gsc_clicks SET DEFAULT 0, ALTER COLUMN gsc_clicks SET NOT NULL,
  ALTER COLUMN gsc_impressions SET DEFAULT 0, ALTER COLUMN gsc_impressions SET NOT NULL,
  ALTER COLUMN contacts SET DEFAULT 0, ALTER COLUMN contacts SET NOT NULL,
  ALTER COLUMN booking_intent SET DEFAULT 0, ALTER COLUMN booking_intent SET NOT NULL,
  ALTER COLUMN unique_visitors_prev SET DEFAULT 0, ALTER COLUMN unique_visitors_prev SET NOT NULL,
  ALTER COLUMN gsc_clicks_prev SET DEFAULT 0, ALTER COLUMN gsc_clicks_prev SET NOT NULL,
  ALTER COLUMN confidence SET DEFAULT 'C', ALTER COLUMN confidence SET NOT NULL,
  ALTER COLUMN cooked_start SET NOT NULL,
  ALTER COLUMN cooked_end SET NOT NULL,
  ALTER COLUMN gsc_start SET NOT NULL,
  ALTER COLUMN gsc_end SET NOT NULL;
...
ALTER TABLE public.dashboard_kpis_snapshot
  ALTER COLUMN visitors_n SET DEFAULT 0, ALTER COLUMN visitors_n SET NOT NULL,
  ALTER COLUMN visitors_prev SET DEFAULT 0, ALTER COLUMN visitors_prev SET NOT NULL,
  ALTER COLUMN pageviews_n SET DEFAULT 0, ALTER COLUMN pageviews_n SET NOT NULL,
  ALTER COLUMN pageviews_prev SET DEFAULT 0, ALTER COLUMN pageviews_prev SET NOT NULL,
  ALTER COLUMN contacts_n SET DEFAULT 0, ALTER COLUMN contacts_n SET NOT NULL,
  ALTER COLUMN contacts_prev SET DEFAULT 0, ALTER COLUMN contacts_prev SET NOT NULL,
  ALTER COLUMN gsc_clicks_n SET DEFAULT 0, ALTER COLUMN gsc_clicks_n SET NOT NULL,
  ALTER COLUMN gsc_clicks_prev SET DEFAULT 0, ALTER COLUMN gsc_clicks_prev SET NOT NULL,
  ALTER COLUMN gsc_impressions_n SET DEFAULT 0, ALTER COLUMN gsc_impressions_n SET NOT NULL,
  ALTER COLUMN gsc_impressions_prev SET DEFAULT 0, ALTER COLUMN gsc_impressions_prev SET NOT NULL,
  ALTER COLUMN is_partial SET DEFAULT false, ALTER COLUMN is_partial SET NOT NULL,
  ALTER COLUMN current_day_partial SET DEFAULT false, ALTER COLUMN current_day_partial SET NOT NULL,
  ALTER COLUMN no_prev_baseline SET DEFAULT false, ALTER COLUMN no_prev_baseline SET NOT NULL,
...
ALTER TABLE public.dashboard_expertises_kpis_snapshot
  ...
  ALTER COLUMN gsc_impressions_n SET DEFAULT 0, ALTER COLUMN gsc_impressions_n SET NOT NULL,
  ALTER COLUMN gsc_impressions_prev SET DEFAULT 0, ALTER COLUMN gsc_impressions_prev SET NOT NULL,
  ALTER COLUMN paid_entries_n SET DEFAULT 0, ALTER COLUMN paid_entries_n SET NOT NULL,
  ALTER COLUMN organic_entries_n SET DEFAULT 0, ALTER COLUMN organic_entries_n SET NOT NULL,
  ALTER COLUMN total_entries_n SET DEFAULT 0, ALTER COLUMN total_entries_n SET NOT NULL,
  ALTER COLUMN is_partial SET DEFAULT false, ALTER COLUMN is_partial SET NOT NULL,
  ALTER COLUMN current_day_partial SET DEFAULT false, ALTER COLUMN current_day_partial SET NOT NULL,
  ALTER COLUMN no_prev_baseline SET DEFAULT false, ALTER COLUMN no_prev_baseline SET NOT NULL,
  ALTER COLUMN label_fr SET NOT NULL,
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Le rôle CI lecture seule peut appeler les RPC dashboard (échantillons du contrat)
-- ─────────────────────────────────────────────────────────────────────────────
GRANT EXECUTE ON FUNCTION public.dashboard_annotations(text) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_article_detail(text, text) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_assisted_quarter() TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_expertises_kpis(text) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_expertises_overview(text, integer) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_expertises_trend(text) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_honoraires_funnel(text) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_intervention_effect(text, date) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_lab_gsc_weekly(integer) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_resources_assisted(text) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_resources_cohorts() TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_resources_kpis(text) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_resources_overview(text, integer) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_resources_trend(text) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_seo_by_query(text, text, integer, integer) TO cooked_ci_ro;
GRANT EXECUTE ON FUNCTION public.dashboard_seo_kpis(text, text) TO cooked_ci_ro;

-- ─────────────────────────────────────────────────────────────────────────────
-- 3. Contract-test de budget (corps prod du 03/09/2026 + 1 entrée)
-- ─────────────────────────────────────────────────────────────────────────────
