-- Aligne visitors_daily sur la définition du KPI « Visiteurs uniques » :
-- anon distincts sur TOUT event de _evc (pageview ∪ page_exit), pas seulement
-- les pageviews. Ainsi sum(visiteurs/jour) ≥ uniques-fenêtre (relation correcte
-- pour un métrique unique). pageviews_daily reste sur name='pageview'.
-- Seule la sous-requête visitors_daily change vs dashboard_resources_trend_series.
