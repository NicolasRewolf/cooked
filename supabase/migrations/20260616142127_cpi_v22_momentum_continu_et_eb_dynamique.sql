-- 16/06/2026 — CPI v2.2 : promotion en prod des 2 améliorations validées
-- (revue mathématique externe + tests internes : corr 0.9855 avec v2.1,
--  ZÉRO verdict fiable A/B déplacé de >=5 pts).
--   1) Momentum à TRANSITION CONTINUE : mélange sigmoïde w=σ((c1+c0-20)/5) des
--      régimes "position" (peu de clics) et "clics relatif au site", au lieu de
--      la bascule discrète à c1+c0=20 (défaut de cohérence ordinale pointé par
--      la revue : un saut de 19→20 clics changeait le système d'équations).
--   2) Lissage EB DYNAMIQUE (Beta-Binomial par type, méthode des moments,
--      kappa = m(1-m)/v - 1 borné [5,200]) pour rétention/lecture, au lieu du
--      pseudo-compte fixe 20. Fort lissage si type homogène, faible si dispersé.
-- INCHANGÉS : capture, conversion, standardisation médiane/MAD, gate LCP,
-- agrégation (sigmoïde, poids 0.30/0.15/0.20/0.35). Signature identique →
-- cpi_daily, cpi_movers, alerte cpi_drop reprennent la formule automatiquement.
...
WITH fit AS (
  ...
  FROM (SELECT round(position)::int pos, (sum(clicks)+1.0)/(sum(impressions)+20.0) ctr
        FROM public.gsc_query_page_daily WHERE day > current_date - 90 AND query !~* 'plouton'
...
capq AS (SELECT g.path, sum(g.impressions) i_qpd,
  ...
  FROM public.gsc_query_page_daily g, fit f WHERE g.day > current_date - p_days AND g.query !~* 'plouton' GROUP BY g.path),
...
ex2 AS (SELECT o.path, public.cooked_page_type(o.path) ptype, (e.props->>'duration_seconds')::numeric d,
    coalesce((e.props->>'max_scroll')::numeric,0) s,
    ((e.props->>'duration_seconds')::numeric >= 15 OR coalesce(s2.pv,1) >= 2) retained
  FROM orge o JOIN public.events_human e ON e.session_id=o.session_id AND e.path=o.path AND e.name='page_exit'
...
-- EB dynamique : pseudo-compte kappa par type (Beta-Binomial, méthode des moments)
ebk AS (
...
DROP FUNCTION IF EXISTS public.cooked_page_index_v22(integer);
DROP FUNCTION IF EXISTS public.cooked_page_index_v23(integer);
