-- T-20fix (03/07/2026) — scope officiel des 14 pages expertise du dashboard
-- ============================================================================
-- Bug remonte par Nicolas : l'onglet /expertises montrait detention-provisoire
-- et garde-a-vue (pages RETIREES du site, en 301) et il manquait droit-penal
-- et proces-criminel (les 2 plus grosses pages penales, vivantes en 200).
-- Cause : l'enumeration depuis page_taxonomy — l'heuristique historique
-- n'inserait que les slugs matchant un theme ; droit-penal et proces-criminel
-- n'en matchaient aucun et n'ont jamais ete dans la table. Le "14" comptait
-- juste, mais pas les bons.
-- Fix : (1) les 2 pages manquantes entrent dans page_taxonomy (source manual) ;
-- (2) le scope du refresh devient la liste business explicite des 14.
-- ============================================================================

INSERT INTO public.page_taxonomy (path, theme, source)
VALUES ('/defense-penale/droit-penal', 'procédure pénale', 'manual'),
       ('/defense-penale/proces-criminel', 'procédure pénale', 'manual')
