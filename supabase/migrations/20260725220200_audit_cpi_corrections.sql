-- Audit 25/07/2026 — tâche #9 : CPI (momentum non-brandé, convertit réel, snapshot).

ALTER TABLE public.cpi_daily
  ADD COLUMN IF NOT EXISTS convertit boolean;

-- Momentum sur clics non-brandés ; convertit = val organique > 0 (pas zv > 0).
DROP FUNCTION IF EXISTS public.cooked_page_index(integer);
...
COMMENT ON VIEW public.cpi_opportunite_contact IS
  'Opportunité de contact : convertit = contact organique réel (val>0), pas zv>0.';
