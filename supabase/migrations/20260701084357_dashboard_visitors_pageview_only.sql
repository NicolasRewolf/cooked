-- Fix: les VISITEURS du dashboard ressources sont comptés sur pageview only
-- (au lieu de pageview+page_exit). Motif: le 01/07/2026 un swarm de bots a émis
-- ~4200 page_exit sans pageview sur les pages ressources ; le refresh de 10:00
-- (avant que noise_sessions ne les flagge) a gelé un faux pic (visitors_daily du
-- jour = 1257 vs ~82 réel ; KPI 28j 10799 vs ~9421). Un visiteur sans pageview
-- n'est pas un visiteur -> COUNT(DISTINCT anonymous_id) FILTER (WHERE name='pageview')
-- pour visitors_n, visitors_prev et la série visitors_daily. page_exit CONSERVÉ
-- pour dwell/scroll ; pageviews_* déjà en pageview-only. Base: 20260630181347.
    ...
    SELECT w,
      ...
      (SELECT array_agg(COALESCE(ct.ct,0)::numeric ORDER BY ds.d)
         ...
         LEFT JOIN (
           ...
           FROM events e JOIN page_taxonomy pt ON pt.path=e.path AND pt.category='ressource'
