-- ⚠️ DÉPRÉCIÉ (28/05/2026) — utiliser supabase/migrations/20260526100000_cooked_period_bounds.sql
-- Ce fichier n'est plus maintenu (risque de drift). Voir supabase/scripts/README.md
-- ============================================================
-- COPIER TOUT CE FICHIER dans Supabase → SQL Editor → Run
-- ============================================================

DROP FUNCTION IF EXISTS public.site_kpis_compare(integer);
DROP FUNCTION IF EXISTS public.site_kpis_compare(text);
DROP FUNCTION IF EXISTS public.site_pulse(integer, integer, numeric);
DROP FUNCTION IF EXISTS public.site_pulse(text, numeric);
DROP FUNCTION IF EXISTS public.site_seo_funnel(integer);
DROP FUNCTION IF EXISTS public.site_seo_funnel(text);
DROP FUNCTION IF EXISTS public.gsc_pages_compare(integer);
DROP FUNCTION IF EXISTS public.gsc_pages_compare(text);
DROP FUNCTION IF EXISTS public.cooked_pages_compare(integer);
DROP FUNCTION IF EXISTS public.cooked_pages_compare(text);
DROP FUNCTION IF EXISTS public.pages_pulse(integer, integer, numeric);
DROP FUNCTION IF EXISTS public.pages_pulse(text, numeric);
DROP FUNCTION IF EXISTS public.pages_overview_unified(integer);
DROP FUNCTION IF EXISTS public.pages_overview_unified(text, integer);
DROP FUNCTION IF EXISTS public.gsc_top_queries_global(integer, integer);
DROP FUNCTION IF EXISTS public.gsc_top_queries_global(text, integer);
DROP FUNCTION IF EXISTS public.gsc_x_dfs_opportunities(integer, numeric, numeric, integer, integer);
DROP FUNCTION IF EXISTS public.gsc_x_dfs_opportunities(integer, numeric, numeric, text, integer);
DROP FUNCTION IF EXISTS public.gsc_page_performance(text);
DROP FUNCTION IF EXISTS public.gsc_page_performance(text, text);
DROP FUNCTION IF EXISTS public.macro_contacts_by_path(integer);
-- ============================================================
-- 1. cooked_period_bounds(period_kind)
-- ============================================================
CREATE OR REPLACE FUNCTION public.cooked_period_bounds(period_kind text)
RETURNS TABLE (
...
BEGIN
  ...
  CASE v_kind
    ...
    WHEN 'week' THEN
      v_n_start := date_trunc('week', v_today::timestamp)::date;
      v_prev_start := v_n_start - 7;
      v_prev_end := v_n_end - 7;
      v_label := 'Semaine en cours';

    WHEN 'month' THEN
      v_n_start := date_trunc('month', v_today::timestamp)::date;
...
COMMENT ON FUNCTION public.cooked_period_bounds(text) IS
  'Bornes N et N-1 en calendrier Paris : today, week (lun→auj), month (1er→auj), rolling_28, rolling_90.';

REVOKE EXECUTE ON FUNCTION public.cooked_period_bounds(text) FROM public, anon, authenticated;
GRANT  EXECUTE ON FUNCTION public.cooked_period_bounds(text) TO service_role;


-- ============================================================
-- 2. macro_contacts_by_path(start_date, end_date)
-- ============================================================
DROP FUNCTION IF EXISTS public.macro_contacts_by_path(integer);
...
AS $$
  SELECT
    ...
    count(*) FILTER (WHERE e.name = 'form_submit')::bigint,
    (
...
COMMENT ON FUNCTION public.macro_contacts_by_path(date, date) IS
  'Contacts macro (phone + form) et booking_intent par path, fenêtre [start_date, end_date] Paris.';
...
-- Rétrocompat : RPCs legacy (gsc_pages_overview v5, etc.)
...
-- ============================================================
-- 3. site_kpis_compare(period_kind) — remplace integer overload
-- ============================================================
DROP FUNCTION IF EXISTS public.site_kpis_compare(integer);
...
COMMENT ON FUNCTION public.site_kpis_compare(text) IS
  'KPIs business N vs N-1 (period_kind). Bornes Paris via cooked_period_bounds.';
...
-- ============================================================
-- 5. gsc_pages_compare / cooked_pages_compare(period_kind)
-- ============================================================
DROP FUNCTION IF EXISTS public.gsc_pages_compare(integer);

CREATE OR REPLACE FUNCTION public.gsc_pages_compare(period_kind text DEFAULT 'rolling_28')
RETURNS TABLE (
  path                    text,
  clicks_n                bigint,
  clicks_prev             bigint,
...
REVOKE EXECUTE ON FUNCTION public.gsc_pages_compare(text) FROM public, anon, authenticated;
GRANT  EXECUTE ON FUNCTION public.gsc_pages_compare(text) TO service_role;
...
-- ============================================================
-- 6. pages_pulse / site_pulse(period_kind) — même fenêtre GSC + Cooked
-- ============================================================
DROP FUNCTION IF EXISTS public.pages_pulse(integer, integer, numeric);
  ...
  WITH g AS (
    SELECT gpc.*,
      CASE
        ...
        ELSE 'flat'
      END AS gsc_dir
    FROM public.gsc_pages_compare(period_kind) gpc
  ),
  c AS (
    SELECT cpc.*,
      CASE
        ...
        ELSE 'flat'
      END AS cooked_dir
    FROM public.cooked_pages_compare(period_kind) cpc
  ),
...
-- ============================================================
-- 7. pages_overview_unified(period_kind, max_rows) v6
-- ============================================================
DROP FUNCTION IF EXISTS public.pages_overview_unified(integer);
...
AS $$
  WITH bounds AS (
    SELECT * FROM public.cooked_period_bounds(period_kind) LIMIT 1
  ),
  ...
  all_paths AS (
    SELECT path FROM public.seo_url_snapshot
    UNION
    SELECT DISTINCT g.path FROM public.gsc_path_daily g, b
    WHERE g.day >= b.n_start AND g.day <= b.n_end
    UNION
    SELECT DISTINCT e.path FROM public.events_human e, b, ts
    WHERE e.path IS NOT NULL
  ...
  mc AS (
    SELECT m.* FROM public.macro_contacts_by_path(
  ...
  SELECT
    ...
    (c.path IS NOT NULL)
  FROM all_paths ap
    LEFT JOIN gsc_n g ON g.path = ap.path
    LEFT JOIN cooked c ON c.path = ap.path
    LEFT JOIN pogo pg ON pg.path = ap.path
    LEFT JOIN mc ON mc.path = ap.path
  ORDER BY coalesce(c.sessions, 0) DESC, coalesce(g.clicks_total, 0) DESC, ap.path
  LIMIT max_rows;
$$;
...
-- ============================================================
-- 8. gsc_top_queries_global / gsc_x_dfs_opportunities(period_kind)
-- ============================================================
DROP FUNCTION IF EXISTS public.gsc_top_queries_global(integer, integer);
...
-- ============================================================
-- 9. gsc_page_performance(target_path, period_kind)
-- ============================================================
DROP FUNCTION IF EXISTS public.gsc_page_performance(text);
