...
Navigateur (Wix)          Wix Automations (server-side)
  ...
  │  ~133 routines SQL publiées (`supabase/rpcs.sql` = miroir lisible) │
  └───────────────────────────────────────────────────────────────────┘
      ↓
...
**137 routines** (133 Cooked + 4 `unaccent`) documentées dans `supabase/rpcs.sql`, régénéré depuis la prod en CI.
