...
| Sprint | Date | Scope |
|---|---|---|
| **35 — Anchor chrome filter** | 03/06/2026 | `cta_anchor_click` ne compte plus le chrome UI. Le sticky-fallback du tracker (§4c) comptait comme micro-conversion **tout** clic dans un conteneur sticky/fixed. Audit prod : **~90 % des `cta_anchor_click` (6 629 / 7 318) étaient du chrome** — bandeau **Cookiebot** (`#CybotCookiebotDialog` : boutons « Tout autoriser »/« Refuser »/« Personnaliser »/« Autoriser la sélection » **+ clics sur le corps du dialog**), burger, liens de nav, et **dumps de texte** (clic sur un conteneur → libellé = `<script>` inline, méga-menu, liste d'indicatifs téléphoniques…). Fix tracker (`sprint35`) : garde `isAnchorNoise()` (sélecteur Cookiebot + denylist) sur tous les chemins d'émission, **cap de longueur ≥ 80** (les vrais libellés font ≤ 78), et règle structurelle (un vrai anchor porte un `#hash`/`data-anchor`, un lien de nav jamais). Rétroactif : migration `anchor_click_exclude_chrome` — helper `cooked_is_chrome_anchor(props)` + exclusion dans `events_human` (donnée mal typée = bruit, hors base canonique ; à distinguer de `form_submit_counts_as_macro` qui filtre au comptage car correctement typé). 90,6 % nettoyé rétroactivement sans jeter un vrai anchor (vérifié : `events_human` 7 318 → 689) ; résidu ~343 libellés de nav courts ambigus, traités forward par le tracker. Logique vérifiée contre le DOM live + données prod avant déploiement. |
