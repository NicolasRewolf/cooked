...
Navigateur (Wix)          Wix Automations (server-side)
  ...
  │  ~130 routines SQL publiées (`supabase/rpcs.sql` = miroir lisible) │
  └───────────────────────────────────────────────────────────────────┘
      ↓
...
**134 routines** (130 Cooked + 4 `unaccent`) documentées dans `supabase/rpcs.sql`, régénéré depuis la prod en CI.
